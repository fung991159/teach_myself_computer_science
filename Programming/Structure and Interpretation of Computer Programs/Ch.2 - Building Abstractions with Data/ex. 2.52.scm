...
; (for example, by using only one copy of the up-
...
; as to assemble the corners in a different pattern. (For example, you might make 
...
(let ((combine4 (square-of-four identity
                                flip-horiz 
                                flip-vertical
                                ; tb need to be lambda since it is passing two opertation in one higher order function
                                (lambda (p) (flip-vertical (flip-horiz p)))  
                )
    )
