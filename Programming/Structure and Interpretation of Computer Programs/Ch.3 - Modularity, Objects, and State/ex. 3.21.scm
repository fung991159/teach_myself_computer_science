Exercise 3.21: Ben Bitdiddle decides to test the queue implementation described
above. He types in the procedures to the Lisp interpreter and proceeds to try
them out:
...
>> basically we just start from queue's front pointer and print everythig from there, cater for null of course

(define (print-q queue)
    (define (get-queue q)
      (cond ((not (null? q))
              (cons (car q) (get-queue (cdr q)))
      )
    ))
    (get-queue (front-ptr queue))