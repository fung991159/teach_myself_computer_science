...
(cdr x)
;<response>
; >> append will not change the original object, hence it stay the same
; as (a b), so (cdr (a b)) will give (b)
(define w (append! x y))
w
;(a b c d)
(cdr x)
;<response>
; >> (b c d)
; >> basically append! will mutate original x object and add (c d) in to cdr of x
; >> so x would in the end equal to z
What are the missing <response>s? Draw box-and-pointer diagrams to explain your
answer.