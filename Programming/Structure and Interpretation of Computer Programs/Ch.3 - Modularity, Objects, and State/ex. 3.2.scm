...
procedure is called during the course of a computation. 
Write a procedure make-monitored that takes
as input a procedure, f, that itself takes one input.
The result returned by make-monitored is a third
...
;; my attmpet to this question, somehow need to "display" the function call
(define (make-monitored f)
  (let ((cnt 0))
    (lambda (x)
      (if (symbol? x)
        (cond ((eq? x 'how-many-calls) cnt)
              ((eq? x 'reset-count) (begin (set! cnt 0) "reset count done"))
